...
// TestBuildResponse_IsByteResponse_EmptyBody_StreamOutput pins down what an
// empty byte response does when the body is streamed to a file.
//
// The empty-body guard from #232 short-circuits before the body is read, and
// whether that short-circuit also skips the streaming call is the one place
// where the two ways of fixing #246 differ. Skipping it leaves no file behind
// and never writes StreamOutputEOFSymbol, so a consumer watching the file for
// that marker waits forever. The body-read has to happen for byte responses
// even when there is nothing to read.
func TestBuildResponse_IsByteResponse_EmptyBody_StreamOutput(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/octet-stream")
		w.Header().Set("Content-Length", "0")
		w.WriteHeader(http.StatusOK)
	...
	_, cErr := BuildResponse("", false, resp, nil, RequestInput{
		IsByteResponse:        true,
	...
	if err != nil {
		t.Fatalf("expected the stream file to exist even for an empty body: %v", err)
	}

	if string(written) != eofSymbol {
		t.Fatalf("stream file content: want %q, got %q", eofSymbol, string(written))
	}
}
